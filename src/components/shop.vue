  ...
  <div class="info" id="bottom">
    ...
    <ul>
      <li
        ...
        v-for="(item,index) in pubtitlelist"
        :key="index"
        @click="title(item.id)"
      >
        <span class="spread-location">{{item.address}}</span>
        <span class="spread-public">公开招标</span>
        <!-- <span class="spread-gs" @click="info">河南华润电力首阳山有限公司2018</span> -->
        <p class="spread-pub">{{item.type}}</p>
...
<script>
import { selectoption,pubtitleList,threelist,detaillist } from "@/util/api";
...
export default {
  ...
  created() {
    this.provinceactive = this.province[0].value;
    this.secondoption = this.second[0].type;
    this.thirdoption = this.third[0].state;
    pubtitleList().then(e => {
      // console.log(e);
      if (e.code == 200) {
        this.$data.pubtitlelist = e.data.res;
      }
    });
    ...
    console.log("id" + this.provinceactive);
    console.log("type" + this.secondoption);
    console.log("state" + this.thirdoption);
  ...
  methods: {
    ...
    title(k) {
      ...
      detaillist(k).then((res)=>{
        // console.log(res)
        if(res.code==200){
          this.$router.push({path:"/bidinfo",query:{key:res}})
        }
      })
    }
  }
